...
namespace BitBase {
enum Result : uint8_t { R_INVALID, R_UNKNOWN, R_WIN, R_DRAW };
...
unsigned encode_index(Color side, Square w_king, Square w_pawn, Square b_king) {
    ...
    unsigned index = w_king + (b_king << 6) + (side << 12) +
                     (file_of(w_pawn) << 13) +
                     ((rank_of(w_pawn) - RANK_7)
                      << 15);  // -RANK_7 because we don't want to include it
...
void parse_index(unsigned index, Color &side, Square &w_king, Square &w_pawn,
                 Square &b_king) {
    assert(index < MAX_INDEX);
...
Result update_score(const std::vector<Result> &results, unsigned idx) {
    ...
    while (k_moves) {
        ...
        unsigned index = encode_index(~side, side == WHITE ? sq : w_ksq, w_pawn,
                                      side == WHITE ? b_ksq : sq);
    ...
    if (side == WHITE && ~Rank7_Bits & w_pawn) {
        ...
        Square next_psq =
                w_pawn +
                UP;  // Next pawn square; - because of Rank enum ordering
...
Value Endgame<ET_KNBK>::strong_side_score(const Position &pos) const {
    ...
    return Value(
            std::min(int64_t(VALUE_KNOWN_WIN + v), int64_t(VALUE_MATE - 1)));
}
...
Value Endgame<ET_KQKR>::strong_side_score(const Position &pos) const {
    ...
    Value v = (MATERIAL_SCORES.piece_value[QUEEN] -
...
Value Endgame<ET_KRKP>::strong_side_score(const Position &pos) const {
    ...
    return (MATERIAL_SCORES.piece_value[ROOK] -
            MATERIAL_SCORES.piece_value[PAWN])
                   .eg -
...
Value Endgame<ET_KBPsKB>::strong_side_score(const Position &pos) const {
    ...
    const Square furthest_pawn_sq = sq_relative_to_side(
            first_pawn_square(pawns, strong_side), strong_side);
    const Square weak_ksq =
            sq_relative_to_side(pos.square<KING>(weak_side), strong_side);
    ...
    if (on_same_file) {
        ...
        if (file_count == 2) {
            const Square furthest_pawn_2_sq = sq_relative_to_side(
                    (first_pawn_square(pawns & file_bb(f2), strong_side)),
                    strong_side);

            if (edge_distance(furthest_pawn_file, f2) ==
                        1 &&  // if pawns are on neighbouring files
                !has_bit_after_pop(pawns &
                                   file_bb(f2)) &&  // AND there is no other
                                                    // pawn on the file
                ...
                square_color(furthest_pawn_sq) ==
                        square_color(
                                strong_bishop_sq))  // AND the furthest pawn is
                                                    // on the same color as the
                                                    // strong bishop
            {
                const Square block1_sq = make_square(
                        furthest_pawn_file, rank_of(furthest_pawn_sq) + 1);
                const Square block2_sq =
                        make_square(f2, rank_of(furthest_pawn_sq));

                if (weak_ksq == block1_sq &&
                    (weak_bishop_sq == block2_sq ||
                     attacks_bb_by<BISHOP>(
                             weak_bishop_sq,
...
Value Endgame<ET_KQKRPs>::strong_side_score(const Position &pos) const {
    ...
    const Square weak_ksq = pos.square<KING>(weak_side);
    const Square rook_sq = pos.square<ROOK>(
            weak_side);  // Only one rook is present, so square<> will retrieve
                         // the only rook
...
bool Endgame<ET_KmmKm>::is_applicable(const Position &pos) const {
    ...
    return strong_minors_count ==
           ...
           weak_minors_count ==
                   1 &&  // Exactly one minor piece on the weak side
