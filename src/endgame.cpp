...
namespace BitBase {
enum Result : std::uint8_t { R_INVALID, R_UNKNOWN, R_WIN, R_DRAW };
...
Value Endgame<ET_KRKP>::strong_side_score(const Position &pos) const {
  ...
  const Square pawn_sq =
      sq_relative_to_side(pos.square<PAWN>(strong_side), strong_side);
  const Square promotion_sq =
      make_square(file_of(pawn_sq), RANK_8);  // Promotion square
...
Value Endgame<ET_KBPsKB>::strong_side_score(const Position &pos) const {
  ...
  if (on_same_file) {
    ...
    while (pp) {
      ...
      if (current_pawn_file != furthest_pawn_file) f2 = current_pawn_file;
