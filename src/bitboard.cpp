...
std::int32_t square_distance[SQUARE_TOTAL][SQUARE_TOTAL];
        ...
        for (PieceType pt : {BISHOP, ROOK}) {
            for (Square y = A8; y <= H1; ++y) {
                if (pseudo_attacks[pt][x] & y) {
                    full_line_bb[x][y] =
                        (attacks_bb_by(pt, x, 0) & attacks_bb_by(pt, y, 0)) | x | y;
                    between_points_bb[x][y] = (attacks_bb_by(pt, x, square_to_BB(y)) &
...
/// Here is used fancy magic bitboard approach
/// </summary>
