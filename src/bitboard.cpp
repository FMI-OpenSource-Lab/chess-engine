        ...
        for (PieceType pt : {BISHOP, ROOK})
            for (Square y = A8; y <= H1; ++y) {
                if (pseudo_attacks[pt][x] & y) {
                    full_line_bb[x][y] = (attacks_bb_by(pt, x, 0) &
                                          attacks_bb_by(pt, y, 0)) |
                                         x | y;
                    between_points_bb[x][y] =
                            (attacks_bb_by(pt, x, square_to_BB(y)) &
                             attacks_bb_by(pt, y, square_to_BB(x)));
                }
...
void init_magics(PieceType pt, SMagic magics[]) {
    ...
    for (Square s = A8; s <= H1; ++s) {
        ...
        sm.magic =
                pt == BISHOP ? BISHOP_MAGIC_NUMBERS[s] : ROOK_MAGIC_NUMBERS[s];
        sm.shift =
                pt == BISHOP ? RELEVANT_BISHOP_BITS[s] : RELEVANT_ROOK_BITS[s];

        // Using Carry-Rippler trick to enumerate through all subsets of the
        // mask
        subset = 0;
        do {
            magic_index = subset;
            ...
            (  // pick between rook or bishop tables
                    pt == BISHOP ? mBishopAttacks[s][magic_index]
                                 : mRookAttacks[s][magic_index]) =
                    sliding_attacks(pt, s, subset);

            // Steps to derive the expression for enumerating the subsets in the
            // set 'mask' First we set all 'unused' bits of the set OR-ing the
            // one's complement of 'mask' The increment ripples a possible carry
            // through all unused bits and then clear all the unused bits by
            // AND-ing (intersection) the set 'mask'

            // subset = ((subset | ~mask) + 1) & mask
            // replace OR with + and make one's complement to two's complement
            // minus one
...
BITBOARD sliding_attacks(PieceType pt, Square s, BITBOARD occ) {
    ...
    for (Direction d : (pt == ROOK ? rook_dirs : bishop_dirs)) {
        ...
        while (is_square_ok(source + d) &&
               square_distance[source][(source + d)] <= 2) {
            source += d;
            attacks |= source;
