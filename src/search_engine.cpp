...
Value SearchEngine::search(std::int32_t depth, SearchInfo &info) {
  ...
  for (std::int32_t current_depth = 1; current_depth <= depth;
    ...
    if (should_stop && current_depth > 1) break;

    // Update PV line if search completed successfully
    if (!should_stop) info.pv = current_pv;
...
Value SearchEngine::negamax(std::int32_t depth, Value alpha, Value beta,
  ...
  // Check for draw
  if (score == VALUE_DRAW) return VALUE_DRAW;
  ...
  if (moves.size() == 0) {
    ...
    return -VALUE_MATE * is_check +
           VALUE_ZERO * (1 - is_check);  // Checkmate or Stalemate
  }
...
Value SearchEngine::quiescence(Value alpha, Value beta, SearchInfo &info) {
  ...
  for (const ScoredMoves &scored_move : legals) {
    if (!is_capture(scored_move)) continue;
...
void SearchEngine::update_pv(std::vector<Move> &pv, Move move,
...
bool SearchEngine::is_capture(Move move) {
  return pos.get_piece_on(move.source_square()) != NO_PIECE ||
