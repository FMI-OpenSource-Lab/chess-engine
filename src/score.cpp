...
namespace {
Score psqt_bonus(PieceType pt, Square s, Color us) {
    return PSQT[pt][sq_relative_to_side(s, us)];
}
    ...
    while (rooks) {
        ...
        if (Component == SC_PIECE_COORDINATION || Component == SC_ALL) {
            ...
            if (same_file || same_rank) {
                score += PIECE_SCORES.connected_rooks;
            }
        }
    }
...
Score score_king_safety(const Position& pos) {
    ...
    // Best shelter among the current king square and any castling
    ...
    if (pos.can_castle(queenside)) {
        shelter = std::max(
            shelter, score_king_shelter<Us>(pos, sq_relative_to_side(C1, Us)),
            compare_scores);
        shelter = std::max(
            shelter, score_king_shelter<Us>(pos, sq_relative_to_side(B1, Us)),
            compare_scores);
    }
...
Score score_king(const Position& pos) {
    ...
    if (Component == SC_MOBILITY || Component == SC_ALL) {
        BITBOARD moves =
            attacks_bb_by<KING>(our_ksq) & ~pos.get_attacked_squares(Us);
    ...
    if (Component == SC_KING_SAFETY || Component == SC_ALL) {
        ...
        if (rank_of(our_ksq) == first_rank && their_rook_and_queen) {
            ...
            BITBOARD blocked =
                ...
                (pos.get_attacks_by<KNIGHT>(Them) | pos.get_attacks_by<BISHOP>(Them) |
...
inline Value Scorer<T>::get_score(const Position& pos) {
    ...
    Value stm_score = pos.side_to_move() == WHITE ? v : -v;
    return TEMPO_ENABLED ? stm_score + TEMPO : stm_score;
}
