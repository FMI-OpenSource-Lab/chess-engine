...
Position &Position::set(const std::string &fen, MoveInfo *mi) {
    ...
    their representive lowercase kq. Uppercase letters mean castling rights for
    White and Lowercase letters mean castling rights for Black. 'K' stands for
    kingside and 'Q' stands for queenside


    4. En passant target square. If there's no en passant target square then the
    notation for this is simplpy '-'. Otherwise the notation is the name of the
    ...
    */
    ...
    while (idx < fen.length() && fen[idx] != ' ') {
        ...
        if (token == 'K')
            ...
            r_sq = make_square(File(token - 'A'),
                               rank_relative_to_side(c, RANK_1));
        else
...
bool Position::gives_check(Move m) const {
    ...
    switch (m.move_type()) {
        ...
        case MT_PROMOTION:
            return attacks_bb_by(m.promoted(), target,
                                 get_all_pieces_bb() ^ source) &
                   opp_king_square;
        ...
        default:  // CASTLING
        {
            ...
            Square rook_target =
                    sq_relative_to_side(target > source ? F1 : D1, us);
...
bool Position::is_legal(Move m) const {
    ...
    if (mt == MT_EN_PASSANT) {
        ...
        BITBOARD occ = get_all_pieces_bb() ^ source ^
                       cap_sq;  // remove our pawn and cap square

        BITBOARD r_att = attacks_bb_by<ROOK>(ksq, occ) &
