...
void normalize(Color strong_side, Color& side, Square& strong_king,
               Square& string_pawn, Square& weak_king);
}  // namespace BitBase

namespace Endgames {
class EndgameBase {
   public:
    EndgameBase(Color strong) : strong_side(strong), weak_side(~strong) {
    }
   ...
   protected:
    // Scores given position from the strong side's perspective, assuming it is
    // applicable
