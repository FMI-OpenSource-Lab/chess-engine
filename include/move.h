...
enum MoveType { MT_NORMAL = 0, MT_PROMOTION, MT_EN_PASSANT, MT_CASTLING };
...
public:
    ...
    // null and none moves
    // has the same source and target square whilst other moves have different
    ...
    constexpr std::uint16_t move_value() const {
        return move;
    }
    constexpr bool is_move_ok() const {
        return invalid_move().move != move && null_move().move != move;
    }

    static constexpr bool same_move(Move a, Move b) {
    ...
    std::string uci_move() const {
        ...
        if (move_type() == MT_CASTLING)
            target = make_square(target > source ? FILE_G : FILE_C,
                                 rank_of(source));
