...
struct Material {
    Score piece_value[PIECE_TYPE_NB] = {
            S(0, 0),       S(300, 370),   S(890, 880), S(900, 950),
            S(1400, 1550), S(2900, 2800), S(0, 0)};
...
struct PieceBonus {
    ...
    Score control_space[PIECE_TYPE_NB] = {S(0, 0),  S(20, 30), S(20, 0),
                                          S(10, 5), S(10, 10), S(10, 20),
                                          S(0, 0)};
...
const Value MAX_PIECE_WEIGHTS =
        4 * (PIECE_WEIGHTS[KNIGHT] + PIECE_WEIGHTS[BISHOP] +
             PIECE_WEIGHTS[ROOK]) +
...
private:
    ...
    Value game_phase_weights(const Position &pos) {
        ...
        w += (pos.count<ROOK>(WHITE) + pos.count<ROOK>(BLACK)) *
             PIECE_WEIGHTS[ROOK];
